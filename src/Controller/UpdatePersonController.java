...
public class UpdatePersonController extends WindowAdapter implements ActionListener, KeyListener,MouseListener  {
    ...
    public void actionPerformed(ActionEvent arg0) {
        ...
        if(arg0.getSource()==view.btnUpdate)
        {
            ...
            int result=information.dialogFensterFragen(view, " Update .....","Inprotuc");
            if(result==0)
            {
                int result2=information.dialogFensterFragen(view, " Sind Sie Sicher ","Inprotuc");
                if(result2==0)
                {
                    ...
                    if((!name.equals(""))&&(!vorname.equals(""))&&(!telefon.equals(""))&&(!gD.equals(""))&&(!fk.equals(""))&&(!email.equals("")))
                    {
                        ...
                        if(statusAbfrage)
                        {
                            JOptionPane.showMessageDialog(view, "Daten Sind bearbeitet");
                            inialilView();
                        }
                    ...
                    else
                    {
                        ...
                        JOptionPane.showMessageDialog(view, "Bitte Prüfen Sie ihre Eingabe Daten ");
                    }
        ...
        //löschen ein Element von der Tablle Aktivität der Student
        if(arg0.getSource()== view.btndelAkt)
        {
            ...
            int res=infoFenster.dialogFensterFragen(view, "Möchten das Element löschen ? ","Inprotuc");
            if(res==0)
            {   GeneralSqlAbfragen sqlAbrage=new GeneralSqlAbfragen();
                ...
                {
                    JOptionPane.showMessageDialog(view, "Zeile ist Gelöscht");
        ...
        //löschen ein Element von der Tablle Status.
        ...
        // löschen ein Element von der Tabelle Bemmerkung:
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if(SpaltenAktivitat=="Status")
        {
            ...
            {	  String altWerte=traget.getValueAt(row, coloumn).toString();
                ...
                String message="Möchten Sie "+traget.getValueAt(row, coloumn)+" änderen";
                int choice=dialog.dialogFensterFragen(view, message, "INPROTUC");
                if(choice==0)
                {	  DialogFenster dialog2=new DialogFenster();
                    String message2 = "Alte Werte "+traget.getValueAt(row, coloumn)+".\nGeben Sie neue Werte ein:";
                    ...
                    String neueValue= dialog2.dialogFensterInput(view , possibilities, defaultoption,message2,"INPROTUC");
                    if(neueValue!=defaultoption)
                    {
                        ...
                        if(statusSql)
                        {
                            ...
                            JOptionPane.showMessageDialog(view, "Versuchen Sie noch Mall Bitte Student kann nicht 2 gleich Status haben :) ");
        ...
        if(SpaltenAktivitat=="Aktivität")
        {
            if((row>=0)&&(coloumn>=0))
            {
                ...
                String message="Möchten Sie  "+altWerte+" änderen";
                int choice=dialog3.dialogFensterFragen(view, message, "INPROTUC");
                if(choice==0)
                {	  DialogFenster dialog2=new DialogFenster();
                    String message2 = "Alte Werte "+traget.getValueAt(row, coloumn)+".\nGeben Sie neue Werte ein:";
                    //++++++++++++++++++to do alle Status von Daten bank abfragen
        ...
        if(SpaltenAktivitat=="Bemerkung")
        {
            if((row>=0)&&(coloumn>=0))
            {
                ...
                {	  DialogFenster dialog2=new DialogFenster();
                    ...
                    String defaultoption=traget.getValueAt(row, coloumn).toString();
                    String neueValue= dialog2.dialogFensterInputText(view,message2,"INPROTUC");
                    if(neueValue!=defaultoption)
                    {
